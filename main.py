...
if __name__ != "__main__":
    ...
    # Checks if user's input is in a provided range. 0 = No, 1 = Yes. Called from payer_setup(), get_ma_type().
    ...
    # Checks if user's input is an integer or not. Returns integer version of input if yes. If no, returns None. Called from payer_setup(), get_ma_type().
    ...
    # Asks user to select a payer. Process is the the same for primary and secondary, so one function is used. Options is numbered dictionary of payers that starts at 1. Returns None to indicate input error. Called from get_ma_type(), payers_input().
    ...
    # Getting the MA type. None is returned to indicate an error from user input. Called from payers_input().
    ...
    # Asking user for input for payers. Payer always starts as None, setup_options is the questions asked of the user, and validation_list is the integers that will be accepted. Called from payers_setup().
    ...
    # Creating email text based on all possible combinations of payers. User input errors from incompatibilities return None. Called from main().
    def output_text(prim_payer, prim_man_payer, sec_payer, sec_man_payer, managing_payers, ma_types, ma_type):
        ...
        if ma_type == 1 and (prim_payer != 5 and (prim_man_payer == None or sec_man_payer == None)): # MSHO MA type selected but MSHO wasn't selected as primary payer AND primary or secondary managing payers are not selected. Choosing MA type MA02 is acceptable ONLY if identical primary/secondary managing insurances were previously selected.
            print("\nIncompatible setup (MSHO requires identical Medicare and Medicaid managing insurances). Please start over.")
            return None
        if ma_type == 1 and (managing_payers[prim_man_payer] != managing_payers[sec_man_payer]): # If MA02 is selected but different primary/secondary managing insurances are selected.
            ...
            return None
        if ma_type == 1: return f"\nFronts are fine. MSHO with {managing_payers[prim_man_payer]}." # MSHO properly selected.
        if ma_type == 2 and prim_payer != 3: # If MA12 was selected but primary payer is anything other than straight MA.
    ...
    def main():
        ...
        # This begins user interaction. Obtains all payer information. Parameters are master payer lists.
        ...
        # This obtains the final email message output
        os.system("cls")
